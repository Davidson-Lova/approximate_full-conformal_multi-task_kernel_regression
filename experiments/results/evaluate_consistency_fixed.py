...
def run(params_path, result_path):
    ...
    for index in [0, 1]:
        ...
        output_covariance_estimator = Covariance(
            mode="fixed",
            covariance_estimator=np.array(params["data"]["covariance_matrix"]),
        )
        ...
        for sample_size, lam in zip(sample_sizes, lams):
            ...
            save_list_with_pickle(
                results, result_path + "result_{}_a.pkl".format(index)
            )


params_path = "params/evaluate_consistency_fixed.json"
result_path = "results/evaluate_consistency/fixed/"
