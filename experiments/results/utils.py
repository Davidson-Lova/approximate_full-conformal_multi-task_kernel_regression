...
def print_results(results):
    ...
    df_results["Non relaxed problem cost"] = df_results[
        "Non relaxed problem cost"
    ].apply(lambda x: f"{x:.3g}")
    ...
    def add_double_border(s):
        ...
        return [
            (
                border_style
                if isinstance(s.index[i], int)
                and i > 0
                and df_results["Perturbation"].iloc[i] == ""
                else ""
            )
    ...
    styled_df = df_results.style.apply(add_double_border, axis=1)
