...
def interpolate_rotations(tab_matrix, weights):
    ...
    assert (
        len(weights) == tab_matrix.shape[0]
    ), "Weights must match the number of matrices."
...
class NonLinearFunction:
    ...
    def get(self, x):
        nonlinear_term = np.sin(np.dot(x, self.beta)) + 0.5 * np.tanh(
            np.dot(x**2, self.beta)
        )
...
class NonLinearFunctionUselessFeatures:
    ...
    def get(self, x):
        ...
        nonlinear_term = np.sin(np.dot(x_usefull, self.beta)) + 0.5 * np.tanh(
            np.dot(x_usefull**2, self.beta)
        )
        return nonlinear_term
...
class LocalPerturbation:
    def __init__(self, d, k, n_anchors=4, radius_transformation=None, seed=42):
        ...
        if radius_transformation is not None:
            ...
            self.radius_transformation = IdTransformation(d)
        self.k = k
        if self.k == 1:
            return
    ...
    def get_one_x(self, x):
        ...
        weights = [
            self.inverse_distance(x, mu) / sum_inv_dist for mu in self.anchors_mu
        ]
...
class LocalPerturbationPSD:
    def __init__(self, d, k, n_anchors=4, radius_transformation=None):
        ...
        if n_anchors == 2:
            self.anchors_mu.append(centers[i])
            self.anchors_cov = [
                np.array([1, -0.9, -0.9, 1]).reshape(2, 2),
                np.array([1, 0.9, 0.9, 1]).reshape(2, 2),
            ]
    ...
    def get_one_x(self, x):
        ...
        cov = np.sum(
            [
                self.inverse_distance(x, mu) / sum_inv_dist * cov
                for mu, cov in zip(self.anchors_mu, self.anchors_cov)
            ],
            axis=0,
        )
        return cholesky(cov)
...
class Perturbation:
    ...
    def get(self, x):
        n = x.shape[0]
        data_perturbation = self.scale_noise * get_perturbation(
            self.noise_type, n, self.k, covariance_matrix=self.covariance_matrix
        )
...
class DataGenerator:
    ...
    def generate_specific_x(self, n):
        if self.bias:
            x_specific = get_perturbation(
                self.law_x, 1, self.d - 1, covariance_matrix=np.eye(self.d - 1)
            )
            x_specific = np.repeat(x_specific, n, axis=0)
            x_specific = np.hstack([np.ones((n, 1)), x_specific])
        else:
            x_specific = get_perturbation(
                self.law_x, 1, self.d, covariance_matrix=np.eye(self.d)
            )
            x_specific = np.repeat(x_specific, n, axis=0)
        ...
        if self.local_perturbation is not None:
            ...
            data_perturbation_specific = np.einsum(
                "njk,nk->nj", rotation, data_perturbation_specific
            )
    ...
    def generate(self, n):
        if self.bias:
            x_train = get_perturbation(
                self.law_x, n, self.d - 1, covariance_matrix=np.eye(self.d - 1)
            )
            x_train = np.hstack([np.ones((n, 1)), x_train])
        else:
            x_train = get_perturbation(
                self.law_x, n, self.d, covariance_matrix=np.eye(self.d)
            )
        ...
        if self.local_perturbation is not None:
            ...
            data_perturbation_train = np.einsum(
                "njk,nk->nj", rotation, data_perturbation_train
            )
    ...
    def generate_perturbation(self, n):
        data_perturbation_train = get_perturbation(
            self.noise_type, n, self.k, covariance_matrix=self.covariance_matrix
        )
        return data_perturbation_train

    def generate_perturbation_train_calibration_test(
        self, n_train, n_calibration, n_test
    ):
        ...
        return (
            data_perturbation_train,
            data_perturbation_calibration,
...
def get_perturbation(perturbation, n_train, k, covariance_matrix=None):
    ...
    if perturbation == "laplace":
        ...
        if covariance_matrix is None:
            raise ValueError(
                "Covariance matrix must be provided for gaussian perturbation"
            )
        data_perturbation_train = np.random.multivariate_normal(
            mean, covariance_matrix, n_train
        )
    elif perturbation == "uniform":
        data_perturbation_train = np.random.uniform(-1, 1, (n_train, k))
    elif perturbation == "pareto":
        data_perturbation_train = np.random.pareto(
            a=2.0, size=(n_train, k)
        )  # a est le paramètre de forme
    elif perturbation == "lognormal":
        data_perturbation_train = np.random.lognormal(
            mean=0.0, sigma=1.0, size=(n_train, k)
        )
        ...
        data_perturbation_train = np.random.chisquare(
            df=2, size=(n_train, k)
        ...
        data_perturbation_train = np.random.gamma(
            shape=2.0, scale=1.0, size=(n_train, k)
        )
    elif perturbation == "beta":
        data_perturbation_train = np.random.standard_t(
            df=3, size=(n_train, k)
        )  # df est le nombre de degrés de liberté
