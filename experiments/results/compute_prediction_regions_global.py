...
def run(params_path, input_path, result_path):
    ...
    for iter in tqdm(range(params["rep_number"])):
        ...
        for regularization in params["regularization_values"]:
            ...
            tac = time()
            result["oracle_prediction_regions"] += [
                {regularization: oracle_prediction_region}
            ]
            result["oracle_prediction_run_times"] += [{regularization: tac - tic}]
            ...
            tac = time()
            result["split_prediction_regions"] += [
                {regularization: split_prediction_region}
            ]
            result["split_prediction_run_times"] += [{regularization: tac - tic}]
            ...
            tac = time()
            result["upper_prediction_regions"] += [
                {regularization: upper_prediction_region}
            ]
            result["upper_prediction_run_times"] += [{regularization: tac - tic}]
...
for index in indices:
    ...
    result_path = "results/prediction_regions/n{}/global/".format(index)
