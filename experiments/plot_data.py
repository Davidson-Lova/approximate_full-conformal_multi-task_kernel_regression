...
def my_add_ellipse(
    ...
    color="red",
    linestyle="-",
    label="Conformal Ellipse",
):
    angles = np.linspace(0, 2 * np.pi, 500)
    unit_circle_points = np.vstack((np.cos(angles), np.sin(angles)))
    circle_points = unit_circle_points * radius

    covariance_root = sqrtm(covariance)
    ellipse_points = (covariance_root @ circle_points).T + center
...
# def add_hyper_rectangle(
#     ax, hyper_rectangle_predictor, x_specific, color="blue", linestyle="-"
# ):
#    """
...
#    for i in range(k):
#       predictions[i, 0] = (
#          hyper_rectangle_predictor.tab_model_alpha_low[i](
#                torch.tensor(x_specific, dtype=torch.float32).reshape(1, -1)
#          ).item()
#          - hyper_rectangle_predictor.conformal_value
#       )

#       predictions[i, 1] = (
#          hyper_rectangle_predictor.tab_model_alpha_high[i](
#                torch.tensor(x_specific, dtype=torch.float32).reshape(1, -1)
#          ).item()
#          + hyper_rectangle_predictor.conformal_value
#       )
...
#     scaling = conformal_value * np.power(
#         np.sum(np.abs(unit_circle) ** q, axis=0), -1 / q
#     )
...
# def old_add_ellipse_multiple_lambdas(
#     ax,
#     f_x,
#     rotation,
#     tab_diag,
...
#         if i > 0:
#             theta = np.linspace(
#                 2 * np.pi * cumulative_weight[i - 1],
#                 2 * np.pi * cumulative_weight[i],
#                 500,
#             )
#         else:
...
#         # Scale the unit circle points to match the conformal value under the q-norm
#         scaling = conformal_value * np.power(
#             np.sum(np.abs(unit_circle) ** tab_q[i], axis=0), -1 / tab_q[i]
#         )
#         scaled_points = unit_circle * scaling
...
#         if i == len(tab_diag) - 1:
#             ax.plot(
#                 [last_point[0], first_point_boucle[0]],
#                 [last_point[1], first_point_boucle[1]],
#                 c="red",
#             )
...
# def add_ellipse_multiple_lambdas(
#     ax,
#     f_x,
#     rotation,
#     tab_diag,
...
#         if j > 0:
#             theta = np.linspace(
#                 2 * np.pi * cumulative_weight[j - 1],
#                 2 * np.pi * cumulative_weight[j],
#                 500,
#             )
#         else:
